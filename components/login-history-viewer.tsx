...
// //   AlertDialogTitle,
// // } from "@/components/ui/alert-dialog"
// // import { formatRelativeTime } from "@/lib/time-utils" // Import time formatting utility
...
// //   lastActivityTime?: string // Added last activity time
...
// //   const [filterSuccess, setFilterSuccess] = useState<string>("all")
// //   const [filterUserType, setFilterUserType] = useState<string>("all")
// //   const [filterOnlineStatus, setFilterOnlineStatus] = useState<string>("all") // Added online status filter
// //   const [statistics, setStatistics] = useState({ totalLogins: 0, successfulLogins: 0, failedLogins: 0 })
// //   const [selectedIds, setSelectedIds] = useState<Set<string>>(new Set())
// //   const [showDeleteDialog, setShowDeleteDialog] = useState(false)
// //   const [deleteTarget, setDeleteTarget] = useState<"single" | "multiple">("multiple")
...
// //       if (filterSuccess !== "all") params.append("success", filterSuccess)
// //       if (filterUserType !== "all") params.append("userType", filterUserType)
// //       if (filterOnlineStatus !== "all") params.append("isOnline", filterOnlineStatus)
...
// //   }, [search, filterSuccess, filterUserType, filterOnlineStatus])
...
// //       h.lastActivityTime ? new Date(h.lastActivityTime).toLocaleString() : "N/A",
// //     ])
...
// //           <CardTitle>Login History</CardTitle>
// //           <CardDescription>
// //             View all login attempts with device info, location, session duration, and real-time online status tracking
// //           </CardDescription>
// //         </CardHeader>
// //         <CardContent>
// //           <div className="flex flex-col gap-4 md:flex-row md:items-center md:justify-between mb-6">
...
// //                           <Clock className="h-4 w-4 text-muted-foreground" />
// //                           <div>
// //                             <div className="font-medium">{formatRelativeTime(record.lastActivityTime)}</div>
// //                             {record.lastActivityTime && (
// //                               <div className="text-xs text-muted-foreground">
// //                                 {new Date(record.lastActivityTime).toLocaleTimeString()}
// //                               </div>
// //                             )}
// //                           </div>
// //                         </div>
...
//                                   Online: {formatRelativeTime(record.lastActivityTime)}
//                                 </div>
//                                 {record.lastActivityTime && (
//                                   <div className="text-xs text-muted-foreground">
//                                     {new Date(record.lastActivityTime).toLocaleString()}
//                                   </div>
//                                 )}
//                               </>
//                             ) : (
//                               <>
//                                 <div className="font-medium">Came Online: {formatRelativeTime(record.timestamp)}</div>
//                                 <div className="text-xs text-muted-foreground">
...
//                                       Went Offline: {formatRelativeTime(record.lastActivityTime)}
//                                     </div>
//                                     <div className="text-xs text-muted-foreground">
//                                       {new Date(record.lastActivityTime).toLocaleString()}
//                                     </div>
...
export default function LoginHistoryViewer({ userRole }: LoginHistoryViewerProps) {
  ...
  // Now only fetches on mount and when filters change
  useEffect(() => {
    fetchHistory()
  }, [search, filterSuccess, filterUserType, filterOnlineStatus])
