...
const allLinks = [
  ...
  },
  {
    ...
    icon: ScanFace,
    roles: ["SuperAdmin", "Admin", "Manager", "Staff", "Teacher"],
  },
  {
    href: "/leave-approval",
  ...
  { href: "/staff", label: "Staff", icon: Users, roles: ["SuperAdmin", "Admin", "Manager"] },
  {
    href: "/notifications",
    label: "Notifications",
    icon: Bell,
  ...
  { href: "/student-attendance", label: "My Attendance", icon: UserCheck, roles: [] },
  { href: "/students", label: "Students", icon: GraduationCap, roles: ["SuperAdmin", "Admin", "Manager", "Teacher"] },
  {
    href: "/leave-requests",
  ...
  { href: "/reports", label: "Reports", icon: BarChart3, roles: ["SuperAdmin", "Admin", "Manager"] },
  {
    href: "/settings",
    label: "Settings",
    icon: Settings,
  ...
  { href: "/super-admin/institutions", label: "Institutions", icon: Building2, roles: ["SuperAdmin"] },
  { href: "/super-admin", label: "Super Admin", icon: ShieldCheck, roles: ["SuperAdmin"] },
]
...
export function Navbar() {
 ...
 useEffect(() => {
  ...
  useEffect(() => {
    ...
    const publicPaths = ["/login", "/contact", "/privacy", "/terms"]
  ...
  useEffect(() => {
    ...
    // Get stored version from localStorage (version when app was last loaded)
    ...
    fetch("/api/app-version")
      ...
      .then((data) => {
        ...
        setLatestVersion(apiVersion)

        console.log("[v0] Version check - Stored:", storedVersion, "API:", apiVersion)

        // Check if versions are different
        if (storedVersion !== apiVersion) {
          console.log("[v0] Version mismatch detected, update available")
          setUpdateAvailable(true)
          // Update stored version after showing update notification
          localStorage.setItem("app_version", apiVersion)
        }
      })
      .catch((err) => {
        console.error("[v0] Failed to fetch app version:", err)
      })

    // Also check if service worker has a waiting worker
    navigator.serviceWorker.ready.then((reg) => {
      if (reg.waiting) {
        console.log("[v0] Service worker update waiting")
        setUpdateAvailable(true)
      }
      ...
      reg.addEventListener("updatefound", () => {
        ...
        newWorker.addEventListener("statechange", () => {
          if (newWorker.state === "installed" && navigator.serviceWorker.controller) {
            console.log("[v0] New service worker installed")
            setUpdateAvailable(true)
          }
        })
      })
  ...
  const { data: notifData, mutate } = useSWR<{ items: Array<{ id: string }> }>(notifQuery, fetcher)

  useEffect(() => {
    if (!user || !mutate) return // Ensure user and mutate are available

    // Refresh notifications when user returns to the app
    const handleVisibilityChange = () => {
      if (document.visibilityState === "visible") {
        mutate() // Refresh when user returns to the app
      }
    }
  ...
  const handleUpdateClick = () => {
    console.log("[v0] Update button clicked")
    setShowUpdateModal(true)
  }

  const handleManualUpdate = () => {
    console.log("[v0] Manual update triggered")
    if (!("serviceWorker" in navigator)) return

    navigator.serviceWorker.ready.then((reg) => {
      if (reg.waiting) {
        console.log("[v0] Sending SKIP_WAITING message to service worker")
        ...
        console.log("[v0] No waiting service worker, reloading page")
        window.location.reload()
      }
    })
  }
  ...
  const { data: leaveCountData } = useSWR<{ count: number }>(
    user && (user.role === "SuperAdmin" || user.role === "Admin" || user.role === "Manager" || user.role === "Teacher")
      ? `/api/leave-requests/pending-count?institutionName=${encodeURIComponent(user.institutionName || "")}`
      : null,
    fetcher,
    { refreshInterval: 30000 }, // Refresh every 30 seconds
  )

  const pendingLeaveCount = leaveCountData?.count || 0

  const handleNotificationClick = () => {
    if (!storageKey || !notifData?.items) return

    const allIds = notifData.items.map((n) => n.id)
    ...
    try {
      ...
      console.error("[v0] Failed to save notification state:", e)
    }
  }
  ...
  const brandName = user?.institutionName?.trim() ? user.institutionName : "Face Attendance"
    ...
    <>
      <nav
        className={cn(
          ...
          scrolled
            ? "border-border/30 bg-background/95 backdrop-blur-xl supports-[backdrop-filter]:bg-background/80 shadow-lg"
            : "border-border/20 bg-background/90 backdrop-blur-xl supports-[backdrop-filter]:bg-background/70 shadow-sm",
        )}
      >
                    ...
                    <Link
                      ...
                      className={cn(
                        ...
                        pathname === l.href || (l.href.startsWith("#") && activeSection === l.href.substring(1))
                          ...
                          : "text-foreground hover:text-teal-600 hover:bg-muted/80",
                      )}
                    >
                      <span className="inline-flex items-center gap-2">
                        ...
                        {l.label === "Leave Approval" && pendingLeaveCount > 0 && (
                          <span className="ml-1 inline-flex items-center justify-center rounded-full bg-orange-600 text-white text-[10px] leading-none h-4 min-w-4 px-1">
                            {pendingLeaveCount}
                          </span>
                        )}
                      </span>
            ...
            <div className="hidden lg:flex items-center space-x-3">
              <DropdownMenu>
                <DropdownMenuTrigger asChild>
                  ...
                  >
                    <Avatar className="h-9 w-9 ring-2 ring-transparent hover:ring-teal-200 dark:hover:ring-teal-500 transition-all duration-200">
                      <AvatarImage
                ...
                <DropdownMenuContent className="w-64 mr-2" align="end" forceMount>
                      ...
                      {user.institutionName && (
                        <p
                          className={cn(
                            ...
                            institutionBlocked ? "text-red-600 dark:text-red-400 font-medium" : "text-muted-foreground",
                          )}
                        >
                  ...
                  <DropdownMenuSeparator />
                  <DropdownMenuItem onClick={handleUpdateClick} className="cursor-pointer group">
                    {updateAvailable ? (
                      <>
                        <Download className="mr-2 h-4 w-4 text-blue-600 group-hover:text-blue-700" />
                        <div className="flex flex-col flex-1">
                          <span className="font-medium text-blue-600">Update Available</span>
                  ...
                  <DropdownMenuItem onClick={toggleTheme} className="cursor-pointer group">
                    {theme === "dark" ? (
                      <>
                        <Sun className="mr-2 h-4 w-4 text-amber-500 group-hover:text-amber-600" />
                        <span className="font-medium">Switch to Light Mode</span>
                      </>
                    ) : (
                      <>
                        <Moon className="mr-2 h-4 w-4 text-blue-600 group-hover:text-blue-700" />
              ...
              <Sheet open={isOpen} onOpenChange={setIsOpen}>
                <SheetTrigger asChild>
                  <Button
                    variant="ghost"
                    size="sm"
                    className="relative h-9 w-9 rounded-xl hover:bg-muted/80 transition-all duration-200 hover:scale-105 active:scale-95"
                  >
                    <MenuIcon className="h-5 w-5 text-foreground transition-transform duration-200" />
                ...
                <SheetContent
                  side="right"
                  className="w-full sm:w-80 bg-background/95 backdrop-blur-xl border-l border-border/50 p-0 flex flex-col"
                >
                  <SheetHeader className="border-b border-border/80 p-4 sm:p-6 flex-shrink-0">
                  ...
                  <div className="flex flex-col flex-1 overflow-y-auto p-4 sm:p-6">
                        ...
                        {primaryLinksMobile.map((l, index) => {
                          ...
                          return (
                            <Link
                              ...
                              className={cn(
                                "group relative flex flex-col items-center justify-center px-3 py-3 rounded-xl text-center text-sm font-medium transition-all duration-300",
                            ...
                            >
                              <IconComponent className="h-5 w-5 mb-1 text-teal-600" />
                              <span className="relative z-10 text-xs">{l.label}</span>
                              {l.label === "Notifications" && unseenCount > 0 && (
                              ...
                              {l.label === "Leave Approval" && pendingLeaveCount > 0 && (
                                <span className="absolute right-2 top-2 inline-flex items-center justify-center rounded-full bg-orange-600 text-white text-[10px] leading-none h-4 min-w-4 px-1">
                                  {pendingLeaveCount}
                                </span>
                              )}
                            </Link>
                    ...
                    <div className="pt-4 border-t border-border/50">
                      <div className="space-y-2 mb-4">
                        ...
                        <button
                          onClick={toggleTheme}
                          className="flex items-center w-full px-4 py-3 text-sm font-medium text-foreground hover:bg-muted rounded-lg transition-colors group"
                        >
                          {theme === "dark" ? (
                            <>
                              <Sun className="mr-3 h-5 w-5 text-amber-500 group-hover:text-amber-600" />
                              <span>Switch to Light Mode</span>
                            </>
                          ) : (
                            <>
                              <Moon className="mr-3 h-5 w-5 text-blue-600 group-hover:text-blue-700" />
                      ...
                      <div className="space-y-2 mb-4">
                        ...
                        <button
                          ...
                          className="flex items-center w-full px-4 py-3 text-sm font-medium hover:bg-muted rounded-lg transition-colors group"
                        >
                          {updateAvailable ? (
                            <>
                              <Download className="mr-3 h-5 w-5 text-blue-600 group-hover:text-blue-700 flex-shrink-0" />
                            ...
                            <>
                              <CheckCircle2 className="mr-3 h-5 w-5 text-green-600 flex-shrink-0" />
                              <div className="flex flex-col items-start flex-1">
                                <span className="font-medium text-green-600">Up to date</span>
                                <span className="text-xs text-muted-foreground">v{currentVersion}</span>
                              </div>
                      ...
                      <div className="space-y-2 mb-4">
                        ...
                        <Link
                          ...
                          className="flex items-center px-4 py-2 text-sm text-foreground hover:bg-muted rounded-lg transition-colors"
                        >
                          Contact Support
                        </Link>
                        <Link
                          href="/privacy"
                          onClick={(e) => handleNavLinkClick("/privacy", e)}
                          className="flex items-center px-4 py-2 text-sm text-foreground hover:bg-muted rounded-lg transition-colors"
                        >
                          Privacy Policy
                        </Link>
                        <Link
                          href="/terms"
                          onClick={(e) => handleNavLinkClick("/terms", e)}
                          className="flex items-center px-4 py-2 text-sm text-foreground hover:bg-muted rounded-lg transition-colors"
                        >
                          Terms of Service
                        </Link>
                      ...
                      <Button
                        onClick={handleLogout}
                        variant="ghost"
                        className="w-full justify-start px-4 py-3.5 h-auto text-red-600 hover:text-red-700 hover:bg-red-50 dark:hover:bg-red-950 rounded-2xl transition-all duration-200 hover:scale-[1.02] active:scale-[0.95]"
                      >
