...
export async function GET(req: NextRequest) {
  try {
    ...
    if (startDate && endDate) {
      ...
      query.date = todayStr() // Default to today if no date specified
    }
    ...
    const allAttendanceForDateQuery: any = {}
    if (startDate && endDate) {
      allAttendanceForDateQuery.date = { $gte: startDate, $lte: endDate }
    } else if (date) {
      allAttendanceForDateQuery.date = date
    } else {
      allAttendanceForDateQuery.date = todayStr()
    }
