...
// import { loadFaceApiModels } from "@/lib/face-api-lazy"
...
// import { HeadMovementDetector } from "@/lib/liveness-detection"
...
//   const [headMovementDetector] = useState(() => new HeadMovementDetector())
//   const lastSpoofResultRef = useRef<Map<string, boolean>>(new Map())
...
//       try {
//         const [faceapi, mpResolver] = await Promise.all([
//           loadFaceApiModels(),
...
//         const detector = await FaceDetector.createFromOptions(mpResolver, {
//           baseOptions: {
//             modelAssetPath:
...
//   useEffect(() => {
//     const savedAntiSpoof = localStorage.getItem("antiSpoofEnabled")
//     if (savedAntiSpoof !== null) {
//       setAntiSpoofEnabled(savedAntiSpoof === "true")
//     }
//   }, [])
...
//       if (antiSpoofEnabled && faceQuality >= 0.6) {
//         try {
...
//           fullCanvas.height = c.height
//           const fullCtx = fullCanvas.getContext("2d")
//           if (!fullCtx) continue
//           fullCtx.drawImage(v, 0, 0, c.width, c.height)
//           const fullImageData = fullCtx.getImageData(0, 0, c.width, c.height)

//           const movementResult = headMovementDetector.detectHeadMovement(
//             { x: sx, y: sy, width: sw, height: sh },
//             fullImageData,
//           )

//           const faceId = `${Math.floor(cx)}_${Math.floor(cy)}`
//           const lastResult = lastSpoofResultRef.current.get(faceId)

//           if (!movementResult.isRealPerson && movementResult.confidence > 0) {
//             ctx.strokeStyle = "rgba(251, 191, 36, 0.95)"
//             ctx.lineWidth = 3
//             ctx.strokeRect(sx, sy, sw, sh)

//             const label = `Spoof Detected (${(movementResult.confidence * 100).toFixed(0)}%)`
//             ctx.fillStyle = "rgba(251, 191, 36, 0.95)"
//             ctx.font = "12px sans-serif"
...
//             if (lastResult !== true) {
//               logMsg(`${movementResult.reason}`)
//               lastSpoofResultRef.current.set(faceId, true)
//             }
//             continue
//           } else if (movementResult.isRealPerson) {
//             if (lastResult === true || lastResult === undefined) {
//               lastSpoofResultRef.current.set(faceId, false)
//             }
//           }
//         } catch (e) {
...
//       } catch (e) {
//         console.error("Descriptor compute failed", e)
//         vec = null
//       }
...
//         const label = `Unknown • L2:${l2Dist.toFixed(3)} • Cosine: ${cosineSim.toFixed(3)}`
...
//           Auto recognition using MediaPipe for detection and face-api.js for descriptors.
//           {antiSpoofEnabled && " Head movement detection enabled to prevent photo/video attacks."}
//         </p>
//       </header>
...
//               <label className="text-sm font-medium">Head Movement Detection</label>
//               <p className="text-xs text-muted-foreground">
//                 Prevent cheating using photos or videos. Detects natural head movements and background consistency.
//               </p>
//             </div>
//             <button
//               onClick={() => {
//                 const newValue = !antiSpoofEnabled
//                 setAntiSpoofEnabled(newValue)
//                 localStorage.setItem("antiSpoofEnabled", String(newValue))
//                 headMovementDetector.reset()
//                 logMsg(`Head movement detection ${newValue ? "enabled" : "disabled"}`)
//               }}
//               className={`relative inline-flex h-6 w-11 items-center rounded-full transition-colors ${
//                 antiSpoofEnabled ? "bg-primary" : "bg-gray-200"
//               }`}
//             >
//               <span
//                 className={`inline-block h-4 w-4 transform rounded-full bg-white transition-transform ${
//                   antiSpoofEnabled ? "translate-x-6" : "translate-x-1"
//                 }`}
//               />
//             </button>
//           </div>
...
export default function FaceIDPage() {
  ...
  return (
    <div className="min-h-screen space-y-3 p-3 pb-24 md:space-y-6 md:p-6 md:pb-6">
      <header className="space-y-1.5">
        <div className="flex flex-col gap-2 sm:flex-row sm:items-center sm:gap-3">
          <h1 className="text-balance text-lg font-semibold md:text-2xl">Face ID Recognition</h1>
          <div className="flex flex-wrap items-center gap-1.5 sm:gap-2">
            {isConnected ? (
              <div className="flex items-center gap-1 px-2 py-0.5 bg-green-100 text-green-800 rounded-full text-[10px] font-medium sm:text-xs sm:py-1">
                <Wifi className="h-2.5 w-2.5 sm:h-3 sm:w-3" />
                Live
              </div>
            ) : (
              <div className="flex items-center gap-1 px-2 py-0.5 bg-red-100 text-red-800 rounded-full text-[10px] font-medium sm:text-xs sm:py-1">
                <WifiOff className="h-2.5 w-2.5 sm:h-3 sm:w-3" />
                Offline
              </div>
            )}
            {currentLocation ? (
              <div className="flex items-center gap-1 px-2 py-0.5 bg-blue-100 text-blue-800 rounded-full text-[10px] font-medium sm:text-xs sm:py-1">
                <MapPin className="h-2.5 w-2.5 sm:h-3 sm:w-3" />
                Location OK
              </div>
            ) : locationLoading ? (
              <div className="flex items-center gap-1 px-2 py-0.5 bg-yellow-100 text-yellow-800 rounded-full text-[10px] font-medium sm:text-xs sm:py-1">
                <MapPin className="h-2.5 w-2.5 sm:h-3 sm:w-3" />
                Getting...
              </div>
            ) : (
            ...
            {realtimeUpdates > 0 && (
              <div className="px-2 py-0.5 bg-blue-100 text-blue-800 rounded-full text-[10px] font-medium sm:text-xs sm:py-1">
                {realtimeUpdates} sent
              </div>
            )}
            {antiSpoofEnabled && (
              <div className="flex items-center gap-1 px-2 py-0.5 bg-purple-100 text-purple-800 rounded-full text-[10px] font-medium sm:text-xs sm:py-1">
                ...
                <span className="hidden sm:inline">Head Movement</span>
                <span className="sm:hidden">Anti-Spoof</span>
              </div>
            )}
          </div>
        </div>
        <p className="text-[11px] leading-relaxed text-gray-600 sm:text-xs md:text-sm">
          Auto recognition using MediaPipe for detection and face-api.js for descriptors.
          {antiSpoofEnabled && " Head movement detection enabled to prevent photo/video attacks."}
        </p>
      </header>
      ...
      <Card>
        <CardHeader className="pb-2 sm:pb-3">
          <CardTitle className="text-base sm:text-lg md:text-xl">Live Camera</CardTitle>
        </CardHeader>
        <CardContent className="space-y-3 sm:space-y-4">
          ...
          <div className="flex flex-col gap-2 sm:flex-row sm:flex-wrap sm:items-center sm:gap-2">
            <Button
              disabled={!modelsReady}
              onClick={startRecognizeLoop}
              className="w-full bg-teal-600 text-white hover:bg-teal-700 sm:w-auto sm:flex-1 md:flex-none text-sm"
              size="sm"
            >
              Start Recognition
            </Button>
            <Button
              ...
              onClick={handleStop}
              disabled={!ready && !running}
              className="w-full sm:w-auto sm:flex-1 md:flex-none text-sm"
              size="sm"
            >
              Stop Camera
            </Button>
            {availableCameras.length > 1 && (
              <Button
                ...
                onClick={switchCamera}
                disabled={running}
                className="w-full bg-transparent sm:w-auto md:w-auto text-sm"
                size="sm"
              >
                <svg className="h-4 w-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                  <path
                ...
                <span className="ml-2">{facingMode === "user" ? "Back Camera" : "Front Camera"}</span>
              </Button>
            )}
          </div>

          <div className="rounded border bg-gray-50 p-2.5 sm:p-3 text-[11px] sm:text-xs text-gray-700">
            <div className="break-words leading-relaxed">
              <span className="font-medium">Status:</span> Camera {ready ? "ready" : "not ready"} • Models{" "}
              {modelsReady ? "ready" : "loading"} • Templates {templates.length} • Loop{" "}
              {running ? "running" : "stopped"}
              <br />
              <span className="font-medium">Camera:</span> {facingMode === "user" ? "Front" : "Back"} •{" "}
              <span className="font-medium">Real-time:</span> {isConnected ? "connected" : "disconnected"}
              {realtimeUpdates > 0 && (
                <>
                  {" "}
                  • <span className="font-medium">{realtimeUpdates}</span> updates sent
                </>
              )}
            </div>
            {log.length > 0 && (
              <ul className="mt-2 list-disc space-y-0.5 pl-4 text-[10px] sm:text-xs">
                {log.map((l, i) => (
      ...
      <Card>
        <CardHeader className="pb-2 sm:pb-3">
          <CardTitle className="text-base sm:text-lg md:text-xl">Recognition Status</CardTitle>
        </CardHeader>
        <CardContent>
          {events.length === 0 ? (
            <div className="rounded-lg border-2 border-dashed border-gray-200 p-6 text-center text-gray-500">
              <div className="mx-auto mb-2 h-10 w-10 sm:h-12 sm:w-12 rounded-full bg-gray-100 flex items-center justify-center">
              ...
              <p className="text-xs sm:text-sm">No recognized records yet.</p>
              <p className="text-[10px] sm:text-xs text-gray-400 mt-1">
                Start recognition to see attendance records here
              </p>
            </div>
          ) : (
            <>
              ...
              <div className="space-y-2.5 sm:space-y-3 lg:hidden">
                {events.map((e, idx) => (
                  <div key={idx} className="rounded-lg border bg-white p-3 sm:p-4 shadow-sm">
                    <div className="mb-2 sm:mb-3 flex items-start justify-between gap-2">
                      <div className="min-w-0 flex-1">
                        <h3 className="text-sm font-medium text-gray-900 truncate sm:text-base">{e.name}</h3>
                      ...
                      <div className="flex-shrink-0">
                        <span
                          className={`inline-flex rounded-full px-1.5 py-0.5 text-[10px] font-medium sm:px-2 sm:py-1 sm:text-xs ${
                            e.status === "present"
                              ? "bg-green-100 text-green-800"
                              : e.status === "late"
                                ? "bg-yellow-100 text-yellow-800"
                    ...
                    <div className="grid grid-cols-2 gap-x-2 gap-y-1.5 text-[11px] sm:text-xs">
                      <div>
                        <span className="text-gray-500">Time:</span>
                        <p className="font-medium truncate">{e.time}</p>
                      </div>
                      {e.department && (
                        <div>
                          <span className="text-gray-500">Department:</span>
                          <p className="font-medium truncate">{e.department}</p>
                        </div>
                      )}
                      {e.role && (
                        <div>
                          <span className="text-gray-500">Role:</span>
                          <p className="font-medium truncate">{e.role}</p>
                        </div>
                      )}
                      {e.shift && (
                        <div>
                      ...
                      {e.className && (
                        <div>
                          <span className="text-gray-500">Class:</span>
                          <p className="font-medium truncate">{e.className}</p>
                        </div>
                      )}
                      <div>
                        <span className="text-gray-500">Match Score:</span>
        ...
        <CardContent className="space-y-3 sm:space-y-4">
          <div className="flex items-start justify-between gap-3">
            <div className="space-y-0.5 flex-1">
              <label className="text-xs font-medium sm:text-sm">Head Movement Detection</label>
            ...
            >
              <span
                className={`inline-block h-5 w-5 sm:h-4 sm:w-4 transform rounded-full bg-white transition-transform ${
                  antiSpoofEnabled ? "translate-x-6 sm:translate-x-6" : "translate-x-1"
                }`}
              />
            </button>
          </div>
