...
// //             <h3 style="color: #1e40af; margin: 0 0 15px; font-size: 18px; font-weight: 600;">📈 Summary Statistics</h3>
// //             <div style="display: grid; gap: 10px;">
...
// //           <!-- Attachments Info -->
// //           <div style="background: linear-gradient(135deg, #ecfdf5, #d1fae5); border-radius: 15px; padding: 25px; margin-bottom: 25px; border-left: 4px solid #10b981;">
// //             <h3 style="color: #065f46; margin: 0 0 15px; font-size: 18px; font-weight: 600;">📎 Report Attachments</h3>
// //             <div style="display: grid; gap: 12px;">
// //               <div style="background: rgba(255, 255, 255, 0.7); border-radius: 8px; padding: 12px; border: 1px solid rgba(16, 185, 129, 0.2);">
...
// //           <!-- Important Notice -->
// //           <div style="background: linear-gradient(135deg, #fef3c7, #fde68a); border-radius: 15px; padding: 20px; margin-bottom: 25px; border-left: 4px solid #f59e0b;">
// //             <h3 style="color: #92400e; margin: 0 0 10px; font-size: 16px; font-weight: 600;">⚠️ Important Notice</h3>
...
export async function sendDataDeletionEmail({
  ...
  to: string
  adminName: string
  dataType: "notifications" | "attendance" | "all"
  deletedBy: string
  ...
  try {
    ...
    const dataTypeLabel =
      dataType === "all" ? "All Data (Notifications + Attendance)" : dataType === "notifications" ? "Notifications Data" : "Attendance Data"
    ...
    <head>
      <meta charset="utf-8">
      <meta name="viewport" content="width=device-width, initial-scale=1.0">
      <title>Data Deletion Notice - Face Attendance</title>
    </head>
    <body style="margin: 0; padding: 0; font-family: 'Segoe UI', Tahoma, Geneva, Verdana, sans-serif; background: linear-gradient(135deg, #667eea 0%, #764ba2 100%); min-height: 100vh;">
      <div style="max-width: 600px; margin: 0 auto; padding: 20px;">
        <div style="background: rgba(255, 255, 255, 0.95); backdrop-filter: blur(10px); border-radius: 20px; padding: 40px; box-shadow: 0 20px 40px rgba(0,0,0,0.1); border: 1px solid rgba(255,255,255,0.2);">
          ...
          <!-- Header -->
          <div style="text-align: center; margin-bottom: 30px;">
            <div style="background: linear-gradient(135deg, #ef4444, #dc2626); width: 80px; height: 80px; border-radius: 50%; margin: 0 auto 20px; display: flex; align-items: center; justify-content: center; box-shadow: 0 10px 30px rgba(239, 68, 68, 0.3);">
              <span style="color: white; font-size: 32px; font-weight: bold;">⚠️</span>
            </div>
            <h1 style="color: #1e293b; margin: 0; font-size: 28px; font-weight: 700;">Data Deletion Notice</h1>
            <p style="color: #64748b; margin: 10px 0 0; font-size: 16px;">Important: Database data has been permanently deleted</p>
          </div>

          <!-- Greeting -->
          <div style="margin-bottom: 30px;">
            <p style="color: #334155; font-size: 18px; margin: 0;">Hello <strong>${adminName}</strong>,</p>
            <p style="color: #64748b; font-size: 16px; margin: 10px 0 0; line-height: 1.6;">This is an automated notification to inform you that database data has been permanently deleted by a SuperAdmin. Below are the details of this action:</p>
          </div>

          <!-- Deletion Details -->
          <div style="background: linear-gradient(135deg, #fee2e2, #fecaca); border-radius: 15px; padding: 25px; margin-bottom: 25px; border-left: 4px solid #ef4444;">
            <h3 style="color: #dc2626; margin: 0 0 15px; font-size: 18px; font-weight: 600;">🗑️ Deletion Details</h3>
            <div style="display: grid; gap: 10px;">
              <div style="display: flex; justify-content: space-between; padding: 8px 0; border-bottom: 1px solid rgba(239, 68, 68, 0.2);">
                <span style="color: #b91c1c; font-weight: 500;">Data Type:</span>
                <span style="color: #dc2626; font-weight: 600;">${dataTypeLabel}</span>
              </div>
              <div style="display: flex; justify-content: space-between; padding: 8px 0; border-bottom: 1px solid rgba(239, 68, 68, 0.2);">
                <span style="color: #b91c1c; font-weight: 500;">Deleted By:</span>
                <span style="color: #dc2626; font-weight: 600;">${deletedBy}</span>
              </div>
              <div style="display: flex; justify-content: space-between; padding: 8px 0; border-bottom: 1px solid rgba(239, 68, 68, 0.2);">
                <span style="color: #b91c1c; font-weight: 500;">Deletion Time:</span>
                <span style="color: #dc2626; font-weight: 600;">${new Date().toLocaleString("en-US", {
                  timeZone: "Asia/Kolkata",
                  year: "numeric",
                  month: "long",
                  day: "numeric",
              ...
              ${
                dataType === "notifications" || dataType === "all"
                  ? `
              <div style="display: flex; justify-content: space-between; padding: 8px 0; border-bottom: 1px solid rgba(239, 68, 68, 0.2);">
                <span style="color: #b91c1c; font-weight: 500;">Notifications Deleted:</span>
                <span style="color: #dc2626; font-weight: 600;">${notificationsCount.toLocaleString()} records</span>
              </div>`
                  : ""
              }
              ${
                dataType === "attendance" || dataType === "all"
                  ? `
              <div style="display: flex; justify-content: space-between; padding: 8px 0;">
                <span style="color: #b91c1c; font-weight: 500;">Attendance Deleted:</span>
                <span style="color: #dc2626; font-weight: 600;">${attendanceCount.toLocaleString()} records</span>
              </div>`
                  : ""
              }
            </div>
          ...
          ${
            (dataType === "attendance" || dataType === "all") && pdfBuffer && excelBuffer
              ? `
          <!-- Backup Data Attachments -->
          <div style="background: linear-gradient(135deg, #dbeafe, #bfdbfe); border-radius: 15px; padding: 25px; margin-bottom: 25px; border-left: 4px solid #3b82f6;">
            <h3 style="color: #1d4ed8; margin: 0 0 15px; font-size: 18px; font-weight: 600;">💾 Data Backup Included</h3>
            <p style="color: #1e40af; margin: 0 0 15px; font-size: 14px; line-height: 1.5;">The deleted attendance data has been exported and attached to this email for your records. Please save these files for future reference.</p>
            <div style="display: grid; gap: 12px;">
              <div style="background: rgba(255, 255, 255, 0.7); border-radius: 8px; padding: 12px; border: 1px solid rgba(59, 130, 246, 0.2);">
                <div style="display: flex; align-items: center; gap: 10px;">
                  <span style="font-size: 24px;">📄</span>
                  <div>
                    <div style="color: #1d4ed8; font-weight: 600; font-size: 14px;">Attendance_Backup.pdf</div>
                    <div style="color: #1e40af; font-size: 12px;">PDF report with charts and statistics</div>
                  </div>
                </div>
              </div>
              <div style="background: rgba(255, 255, 255, 0.7); border-radius: 8px; padding: 12px; border: 1px solid rgba(59, 130, 246, 0.2);">
                <div style="display: flex; align-items: center; gap: 10px;">
                  <span style="font-size: 24px;">📊</span>
                  <div>
                    <div style="color: #1d4ed8; font-weight: 600; font-size: 14px;">Attendance_Backup.xlsx</div>
                    <div style="color: #1e40af; font-size: 12px;">Excel spreadsheet with detailed records</div>
                  </div>
                </div>
              </div>
            </div>
          ...
          <div style="background: linear-gradient(135deg, #fef3c7, #fde68a); border-radius: 15px; padding: 20px; margin-bottom: 25px; border-left: 4px solid #f59e0b;">
            <h3 style="color: #92400e; margin: 0 0 10px; font-size: 16px; font-weight: 600;">⚠️ Important Notice</h3>
            <p style="color: #b45309; margin: 0; font-size: 14px; line-height: 1.5;">This deletion is permanent and cannot be undone. The data has been removed from the active database. If you have any concerns about this action, please contact the SuperAdmin immediately.</p>
          </div>

          <!-- Security Recommendation -->
          <div style="background: linear-gradient(135deg, #ecfdf5, #d1fae5); border-radius: 15px; padding: 20px; margin-bottom: 25px; border-left: 4px solid #10b981;">
            <h3 style="color: #065f46; margin: 0 0 10px; font-size: 16px; font-weight: 600;">✅ Action Recommended</h3>
            <p style="color: #047857; margin: 0; font-size: 14px; line-height: 1.5;">Please save the attached backup files to your secure storage. These files contain all the deleted data and can be used for audit purposes or historical reference.</p>
          </div>
          ...
          <!-- Footer -->
          <div style="text-align: center; padding-top: 20px; border-top: 1px solid #e2e8f0;">
            <p style="color: #64748b; font-size: 14px; margin: 0;">This is an automated security notification from Face Attendance</p>
            <p style="color: #94a3b8; font-size: 12px; margin: 5px 0 0;">Generated on ${new Date().toLocaleString()}</p>
          </div>
        </div>
      </div>
    ...
    const mailOptions: any = {
      from: `${institutionName || "Face Attendance"} <faceattendance@noreply.com>`,
      to,
      subject: `⚠️ Data Deletion Notice - ${dataTypeLabel} Permanently Deleted`,
      html: htmlContent,
    }
    ...
    if (pdfBuffer && excelBuffer) {
      mailOptions.attachments = [
        {
          filename: `Attendance_Backup_${new Date().toISOString().split("T")[0]}.pdf`,
          content: pdfBuffer,
          contentType: "application/pdf",
        },
        {
          filename: `Attendance_Backup_${new Date().toISOString().split("T")[0]}.xlsx`,
          content: excelBuffer,
          contentType: "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet",
        },
      ]
    ...
    console.error("Failed to send data deletion email:", error)
    return false
  }
}
